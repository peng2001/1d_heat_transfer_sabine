...
def run_model(time_record, cells_list, cells_temperatures_init, Temperature_side_minusL, Temperature_side_plusL, dt, dx, Heat_flux, k, rho, cp):
    ...
    for time_index, time in enumerate(time_record):
        ...
        temperatures[time_index] = time_step_calc(cells_list = cells_list, prev_temperatures_list = temperatures[time_index-1], dt=dt, dx=dx, Heat_flux=Heat_flux, k=k, rho=rho, cp=cp)
    return temperatures
...
def time_step_calc(cells_list, prev_temperatures_list, dt, dx, Heat_flux, k, rho, cp):
    ...
    for cell_index, cell_location in enumerate(cells_list):
        dTdt_list[cell_index] = alpha*(d2Tdx2_list[cell_index]+Heat_flux/k)
